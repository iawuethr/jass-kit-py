...
# In contrast to montecarlo.py we return the whole game node 
# and not only the card to be played in the game node.
class MonteCarloTreeSearchIncomplete:
    ...
    def findNextMove(self, game: GameSim, playerNo: int) -> np.array: 
        ...
        while (time.time()*1000.0 < end): 
            ...
            # If the root node has children, then the selectPromisingNode will provide us with some
            # child node of a high-UTC-path.
            ...
            # If the promising node is not a leaf-node (end of game), then we determine all possible children.
    ...
    def selectPromisingNode(self,rootNode: Node) -> Node: 
        ...
        # since no expansion and children creation happened yet.
